...
-(void)touch:(UITapGestureRecognizer*)recogniser
{
    ...
    if (touchPoint.x>=sizeofWindo/3 && touchPoint.x<=(sizeofWindo/2+50))
    {
        if (1==finalcount)
        {
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Guess" message:@"Your Guess is Right" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    ...
    if ((touchPoint.x>=sizeofWindo/2+50) && touchPoint.x<=sizeofWindo)
    {
        if (2==finalcount)
        {
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"Guess" message:@"Your Guess is Right" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
-(void)xDrowOptionMatrix
{
    ...
    NSMutableArray *arrayfrom=[[NSMutableArray alloc]initWithArray:[dict valueForKey:@"displayMatrix"]];
    tempValueArray=[[NSMutableArray alloc]initWithArray:[dict valueForKey:@"tempValueArray"]];
    NSString *strfrom=[dict valueForKey:@"count"];
    ...
    for (int t=0; t<3; t++)
     {        
        ...
        if (t==finalcount)
        {
            [self xDrowImageViewOnScrollview:[strfrom intValue] width:xaxis];
            
          for (NSString * stringArray in arrayfrom)
                {
                    int countvalue=total+[stringArray intValue];
            ...
            if (chekisused) {
                ...
                countmatrix=[[dicta valueForKey:@"count"] intValue];
                chekisused=FALSE;
                }else
                {
                arrayfromdic=[[NSMutableArray alloc]initWithArray:[dicta valueForKey:@"ontherValueTwoArray"]];
                    countmatrix=[[dicta valueForKey:@"count1"] intValue];
                    }
                [self xDrowImageViewOnScrollview:countmatrix width:xaxis];
